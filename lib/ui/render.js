(function registerRenderer(global) {
  ...
  function renderClaimStructure(declaration, locale) {
    return '' +
      '<div class="meta">' +
        '<span>' + escapeHtml(i18n.t("contextScopeLabel", { locale: locale })) + ' ' + escapeHtml(declaration.context.scope || i18n.t("notDefined", { locale: locale })) + '</span>' +
        '<span>' + escapeHtml(i18n.t("evaluationCheckLabel", { locale: locale })) + ' ' + escapeHtml(declaration.evaluation.check || i18n.t("notDefined", { locale: locale })) + '</span>' +
        '<span>' + escapeHtml(i18n.t("evaluationConfirmerLabel", { locale: locale })) + ' ' + escapeHtml(declaration.evaluation.confirmer || i18n.t("notDefined", { locale: locale })) + '</span>' +
      '</div>' +
      (declaration.context.notes || declaration.evaluation.evidence
        ? '<div class="meta">' +
          (declaration.context.notes
            ? '<span>' + escapeHtml(i18n.t("contextNotesLabel", { locale: locale })) + ' ' + escapeHtml(declaration.context.notes) + '</span>'
            : "") +
          (declaration.evaluation.evidence
            ? '<span>' + escapeHtml(i18n.t("evaluationEvidenceLabel", { locale: locale })) + ' ' + escapeHtml(declaration.evaluation.evidence) + '</span>'
            : "") +
        '</div>'
        : "");
  }

  function renderStructureReadiness(structure, locale) {
    if (structure.isStructured) {
      return '<span class="small">' + escapeHtml(i18n.t("claimReady", { locale: locale })) + '</span>';
    }

    return '<span class="small">' + escapeHtml(i18n.t("claimMissingFields", {
      locale: locale,
      params: { count: structure.missing.length }
    })) + '</span>';
  }
  ...
  function renderDeclarationForm(state) {
    ...
    return '' +
      '<form id="create-declaration-form">' +
        '<label>' + escapeHtml(i18n.t("formTitle", { locale: locale })) + '<input name="title" maxlength="80" /></label>' +
        '<label>' + escapeHtml(i18n.t("formDescription", { locale: locale })) + '<textarea name="description" maxlength="240"></textarea></label>' +
        '<label>' + escapeHtml(i18n.t("formContextScope", { locale: locale })) + '<input name="contextScope" maxlength="120" /></label>' +
        '<label>' + escapeHtml(i18n.t("formContextNotes", { locale: locale })) + '<textarea name="contextNotes" maxlength="180"></textarea></label>' +
        '<label>' + escapeHtml(i18n.t("formEvaluationCheck", { locale: locale })) + '<textarea name="evaluationCheck" maxlength="180"></textarea></label>' +
        '<label>' + escapeHtml(i18n.t("formEvaluationConfirmer", { locale: locale })) + '<input name="evaluationConfirmer" maxlength="120" /></label>' +
        '<label>' + escapeHtml(i18n.t("formEvaluationEvidence", { locale: locale })) + '<input name="evaluationEvidence" maxlength="120" /></label>' +
        '<label>' + escapeHtml(i18n.t("formRequestedWeight", { locale: locale })) + '<input name="requestedWeight" type="number" min="1" max="99" value="3" /></label>' +
        '<div class="actions">' +
          '<button type="submit" value="draft">' + escapeHtml(i18n.t("createDraftDeclaration", { locale: locale })) + '</button>' +
          '<button type="submit" class="secondary" value="projected">' + escapeHtml(i18n.t("createProjectedClaim", { locale: locale })) + '</button>' +
        '</div>' +
      '</form>';
  }

  function renderDeclarationEditor(declaration, locale) {
    return '' +
      '<form data-action-form="update-declaration" data-id="' + declaration.id + '">' +
        '<label>' + escapeHtml(i18n.t("formTitle", { locale: locale })) + '<input name="title" maxlength="80" value="' + escapeHtml(declaration.title) + '" /></label>' +
        '<label>' + escapeHtml(i18n.t("formDescription", { locale: locale })) + '<textarea name="description" maxlength="240">' + escapeHtml(declaration.description) + '</textarea></label>' +
        '<label>' + escapeHtml(i18n.t("formContextScope", { locale: locale })) + '<input name="contextScope" maxlength="120" value="' + escapeHtml(declaration.context.scope || "") + '" /></label>' +
        '<label>' + escapeHtml(i18n.t("formContextNotes", { locale: locale })) + '<textarea name="contextNotes" maxlength="180">' + escapeHtml(declaration.context.notes || "") + '</textarea></label>' +
        '<label>' + escapeHtml(i18n.t("formEvaluationCheck", { locale: locale })) + '<textarea name="evaluationCheck" maxlength="180">' + escapeHtml(declaration.evaluation.check || "") + '</textarea></label>' +
        '<label>' + escapeHtml(i18n.t("formEvaluationConfirmer", { locale: locale })) + '<input name="evaluationConfirmer" maxlength="120" value="' + escapeHtml(declaration.evaluation.confirmer || "") + '" /></label>' +
        '<label>' + escapeHtml(i18n.t("formEvaluationEvidence", { locale: locale })) + '<input name="evaluationEvidence" maxlength="120" value="' + escapeHtml(declaration.evaluation.evidence || "") + '" /></label>' +
        '<label>' + escapeHtml(i18n.t("formRequestedWeight", { locale: locale })) + '<input name="requestedWeight" type="number" min="1" max="99" value="' + declaration.requestedWeight + '" /></label>' +
        '<div class="actions">' +
          '<button type="submit" value="draft">' + escapeHtml(i18n.t("saveDraftChanges", { locale: locale })) + '</button>' +
          '<button type="submit" class="secondary" value="projected">' + escapeHtml(i18n.t("saveAsProjected", { locale: locale })) + '</button>' +
        '</div>' +
      '</form>';
  }
      ...
      return '' +
        '<article class="card">' +
          '<div class="card-top">' +
            ...
            '</div>' +
            '<span class="pill' + ((declaration.status === "resolved" || declaration.status === "completed") ? " success" : "") + '">' + escapeHtml(i18n.tStatus(declaration.status, locale)) + '</span>' +
          '</div>' +
          renderClaimStructure(declaration, locale) +
          '<div class="meta">' +
          ...
          '</div>' +
          '<div class="actions">' + renderStructureReadiness(declaration.structure, locale) + '</div>' +
          ((declaration.status === "draft" || declaration.status === "projected")
            ? renderDeclarationEditor(declaration, locale)
            : "") +
          '<div class="actions">' +
            (declaration.status === "projected"
              ...
              : "") +
            ((declaration.status === "draft" || declaration.status === "projected" || declaration.status === "committed") && !hasActiveIntent
            ...
            ? '<div class="list">' + intents.map(function intentRow(intent) {
              ...
              return '' +
                '<div class="card">' +
                  ...
                  '</div>' +
                  '<div class="actions">' +
                    (intent.status === "proposed" && declaration.status === "committed"
      ...
      return '' +
        '<article class="card">' +
          ...
          (declaration.status === "committed"
            ...
            '</form>'
            : '<div class="small">' + escapeHtml(i18n.t("claimUnavailableForProposal", { locale: locale })) + '</div>') +
        '</article>';
    }).join("") + '</div>';
  }
      ...
      return '' +
        '<article class="card">' +
          '<div class="card-top">' +
            ...
            '</div>' +
            '<span class="pill' + (plan.declaration.status === "resolved" ? " success" : "") + '">' + escapeHtml(i18n.tStatus(plan.declaration.status, locale)) + '</span>' +
          '</div>' +
          '<div class="meta">' +
