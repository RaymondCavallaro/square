(function registerI18n(global) {
  ...
  const showdown = vendor.showdown || {};
  ...
  function parseReadmeSummary(markdown) {
    const match = markdown.match(/#+ The Square\n([^#]*)/m);
	const conv = new showdown.Converter() || { makeHtml: function(md) { return md } };

	return conv.makeHtml(match && match[1] || "");
  }
  ...
  async function loadHeroFromReadme(locale) {
    ...
    try {
      ...
      summary.innerHTML = parseReadmeSummary(markdown);
    } catch (error) {
