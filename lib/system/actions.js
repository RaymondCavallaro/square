(function registerActions(global) {
  ...
  function clearByPlan(state, userId) {
    ...
    plans.forEach(function applyPlan(plan) {
      ...
      if (!declaration || declaration.status === "completed" || declaration.status === "canceled") {
        return;
      }

      if (user.availableWeight >= declaration.requestedWeight && declaration.status === "draft") {
  ...
  bus.on("declaration:create", function handleDeclarationCreate(payload) {
    store.setState(function update(state) {
      const declaration = {
        ...
        requestedWeight: Number(payload.requestedWeight),
        committedWeight: 0,
        status: "draft",
  ...
  bus.on("declaration:commit", function handleDeclarationCommit(payload) {
    store.setState(function update(state) {
      ...
      if (!declaration || declaration.ownerId !== user.id || declaration.status !== "draft") {
        return state;
      }
  ...
  bus.on("intent:create", function handleIntentCreate(payload) {
    store.setState(function update(state) {
      ...
      if (!(declaration.status === "committed" || declaration.status === "locked")) {
        return state;
      }
